...
from more_executors import f_proxy
...
class ContentProcessor:
    ...
    def _fetch_out_repo_content(self) -> None:
        ...
        future_rpm_units = search_units(
            self.out_repo, [Criteria.true()], RpmUnit, unit_fields=RPM_FIELDS  # type: ignore
        )
        non_modular_rpms = [
            unit
            for unit in f_proxy(future_rpm_units).result()
            if unit.filename not in self.all_modular_filenames
        ]
        keep_n_latest_rpms(non_modular_rpms)
    ...
    def _fetch_in_repos_contents(self) -> None:
        ...
        fields = ["name", "arch"]
        values = [
            (rpm_unit.name, rpm_unit.arch)
            for rpm_unit in self.out_repo_content.nonmodular_rpm_units
        ]

        search_criteria = create_or_criteria(fields, values)

        for in_repo in self.in_repos:
            future_rpm_units = search_units(
                in_repo, search_criteria, RpmUnit, unit_fields=RPM_FIELDS  # type: ignore
            )
            non_modular_rpms = [
                unit
                for unit in f_proxy(future_rpm_units).result()
